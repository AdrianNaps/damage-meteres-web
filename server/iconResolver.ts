...
const CACHE_FILE = resolve(DATA_DIR, 'spell-icons.json')

// Empty string marks a negative cache entry (lookup failed this session).
...
function scheduleWrite() {
  ...
  writeTimer = setTimeout(() => {
    ...
    try {
      ...
      writeFileSync(CACHE_FILE, JSON.stringify(persist, null, 2))
    } catch (err) {
